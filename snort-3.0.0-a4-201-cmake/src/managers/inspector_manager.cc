...
///////////////////////////////////////////////////////////
//在此处调用各个派生类的eval()函数,由此进入各个协议层进行处理
            (*prep)->handler->eval(p);
    }
}
/////////////////////源码分析如下:
...
struct PHInstance
{
    Inspector* handler;////////////////////////handler 指向Inspector
    .......
};
*/
...
// (*prep)->handler 指向 Inspector 

// (*prep)->handler->eval(p) 调用Inspector的纯虚函数eval()

//Inspector是一个抽象类，它的纯虚函数eval()在派生类中实现
...
//FtpData
//GtpInspect
//HttpInspect----------------------------Http //Http_inspect.cc // 314
//Imap
//InspectorData
//Modbus
//NHttpInspect
